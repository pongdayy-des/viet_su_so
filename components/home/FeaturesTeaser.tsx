    ...
    <section className="bg-cream px-[8vw] py-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {FEATURES.map((f, i) => {
            ...
            >
              ...
              <div className="p-5">
                <div className="flex gap-2 mb-2.5">
                  <span className={`text-[11px] font-semibold px-3 py-1 rounded-full ${f.tag.color}`}>
                    {f.tag.label}
                  </span>
                </div>
                <h3 className="font-serif text-xl font-bold text-navy leading-[1.3] mb-2">{f.title}</h3>
                <p className="text-[13px] text-slate-500 leading-[1.6] mb-4">{f.description}</p>
                <div className="flex justify-between items-center pt-3.5 border-t border-navy/[0.07]">
                  <span className="text-sm text-red-vss font-semibold">{f.cta}</span>
                </div>
              </div>
            </motion.div>
          )
          ...
          if (f.href) {
            return <Link key={f.title} href={f.href} className="no-underline">{CardInner}</Link>
          }
          return (
