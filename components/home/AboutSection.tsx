...
import { SectionEyebrow, SectionTitle } from '@/components/shared/SectionHeader'
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        ...
        <div>
          ...
          <p className="text-base text-slate-500 leading-[1.8] mb-7">
            Việt Sử Số là nền tảng học liệu số khai thác ký ức cộng đồng — được biên tập theo cấu
            trúc sư phạm, phục vụ trực tiếp trong công tác dạy và học lịch sử địa phương tại trường
            phổ thông.
          </p>
          ...
          <div className="flex flex-col gap-4">
            {BENEFITS.map((b, i) => (
              ...
              >
                <div className={clsx('w-10 h-10 rounded-[10px] flex items-center justify-center text-lg flex-shrink-0', ICON_BG[b.color])}>
                  {b.icon}
                </div>
                <div>
                  <h4 className="text-sm font-semibold text-navy mb-0.5">{b.title}</h4>
                  <p className="text-[13px] text-slate-500 leading-[1.5]">{b.description}</p>
                </div>
              </motion.div>
            ))}
          </div>
